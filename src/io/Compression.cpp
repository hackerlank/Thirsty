...
void encodeVarintToIOBuf(uint64_t val, IOBuf* out)
{
    DCHECK_GE(out->tailroom(), kMaxVarintLength64);
    out->append(encodeVarint(val, out->writableTail()));
}

uint64_t decodeVarintFromCursor(Cursor& cursor)
{
    // Must have enough room in *this* buffer.
    auto p = cursor.peek();
    ByteRange range(p.first, p.second);
...
std::unique_ptr<IOBuf> ZlibCodec::addOutputBuffer(z_stream* stream, uint32_t length)
{
...
bool ZlibCodec::doInflate(z_stream* stream, IOBuf* head, uint32_t bufferLength)
{
...
std::unique_ptr<IOBuf> ZlibCodec::doCompress(const IOBuf* data)
{
    ...
    // Max 64MiB in one go
    const uint32_t maxSingleStepLength = uint32_t(64) << 20;    // 64MiB
    const uint32_t defaultBufferLength = uint32_t(4) << 20;     // 4MiB
...
std::unique_ptr<IOBuf> ZlibCodec::doUncompress(const IOBuf* data, uint64_t uncompressedLength)
{
    z_stream stream;
    stream.zalloc = nullptr;
    stream.zfree = nullptr;
...
std::unique_ptr<IOBuf> LZMA2Codec::addOutputBuffer(lzma_stream* stream, size_t length)
{
    CHECK_EQ(stream->avail_out, 0);
    auto buf = IOBuf::create(length);
    buf->append(length);
...
CodecFactory gCodecFactories[static_cast<size_t>(CodecType::NUM_CODEC_TYPES)] =
{
