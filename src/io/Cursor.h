...
public:
  ...
  void clone(std::unique_ptr<IOBuf>& buf, size_t len)
  {
  ...
  void clone(IOBuf& buf, size_t len)
  {
      if (UNLIKELY(cloneAtMost(buf, len) != len))
      {
          throw std::out_of_range("underflow");
  ...
  size_t cloneAtMost(IOBuf& buf, size_t len)
  {
      buf = IOBuf();
  ...
  size_t cloneAtMost(std::unique_ptr<IOBuf>& buf, size_t len)
  {
      if (!buf)
      {
...
class RWCursor
 ...
 public:
  ...
  void insert(std::unique_ptr<IOBuf> buf)
  {
      IOBuf* nextBuf;
      if (this->offset_ == 0)
      {
      ...
      {
          std::unique_ptr<IOBuf> remaining;
