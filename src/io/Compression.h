...
public:
     ...
     */
    std::unique_ptr<IOBuf> compress(const IOBuf* data);
     ...
     * an empty IOBuf chain will return an empty IOBuf chain.
     */
    static const uint64_t UNKNOWN_UNCOMPRESSED_LENGTH = uint64_t(-1);
...
private:
    ...
    virtual std::unique_ptr<IOBuf> doCompress(const IOBuf* data) = 0;
    virtual std::unique_ptr<IOBuf> doUncompress(const IOBuf* data,
...
const int COMPRESSION_LEVEL_DEFAULT = -2;
const int COMPRESSION_LEVEL_BEST = -3;
