...
class BaseFormatter {
 ...
 private:
  ...
  static const size_t valueCount = std::tuple_size<ValueTuple>::value;
 ...
 */
template <class... Args>
void format(std::string* out, StringPiece fmt, Args&&... args) {
 ...
 */
template <class Container>
void vformat(std::string* out, StringPiece fmt, Container&& container) {
