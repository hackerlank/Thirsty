...
inline uint32_t fnv32_buf(const void* buf, int n, uint32_t hash = FNV_32_HASH_START)
{
...
inline uint32_t fnv32(const std::string& str, uint32_t hash = FNV_32_HASH_START)
{
...
inline uint64_t fnv64_buf(const void* buf, int n, uint64_t hash = FNV_64_HASH_START)
{
    const char* char_buf = reinterpret_cast<const char*>(buf);
...
public:
    ...
    {
        return ::hash::hash_combine(x.first, x.second);
    }
};
