...
uint64_t DataHolder::hash(size_t size) const
{
    ...
    uint64_t h = ::hash::fnv64_buf(data_.get(), size);
    hashCache_[size] = h;
    return h;
}
...
{
    uint64_t h = ::hash::FNV_64_HASH_START;
    for (auto& range : *buf)
    {
        h = ::hash::fnv64_buf(range.data(), range.size(), h);
    }
    return h;
}
...
const size_t dataSizeLog2 = 27;  // 128MiB
...
{
    const uint64_t uncompressedLength = 42;
