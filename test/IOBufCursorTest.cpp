...
using io::Cursor;
using io::RWPrivateCursor;
using io::RWUnshareCursor;
using io::Appender;
using io::QueueAppender;
...
void append(std::unique_ptr<IOBuf>& buf, StringPiece data) 
{
...
TEST(IOBuf, Gather)
{
    ...
    EXPECT_EQ("lo world",
        StringPiece(reinterpret_cast<const char*>(cursor.data()),
        cursor.length()));
